    ...
    public Pair<OpenGLMatrix,String> scanTargets(){ //returns null if none seen, otherwise returns location matrix
        ...
        for (VuforiaTrackable trackable : allTrackables) {
            if (((VuforiaTrackableDefaultListener)trackable.getListener()).isVisible()) {
                targetName = trackable.getName(); //will be null if this fails
                //telemetry.addData("Visible Target", trackable.getName());
                // getUpdatedRobotLocation() will return null if no new information is available since
                // the last time that call was made, or if the trackable is not currently visible.
                OpenGLMatrix robotLocationTransform = ((VuforiaTrackableDefaultListener)trackable.getListener()).getUpdatedRobotLocation();
                Pair<OpenGLMatrix,String> ans = new Pair<OpenGLMatrix, String>(robotLocationTransform,targetName);
                return ans ; // will be null if undefined
